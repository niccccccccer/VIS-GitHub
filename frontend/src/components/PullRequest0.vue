  ...
  >
    <n-grid :cols="1" style="height: 4%">
      <n-gi span="1">
        <n-grid :cols="6">
          <n-gi span="5">
            <n-input placeholder="query" v-model:value="query" type="text" path="query">
              <template #suffix>
                <n-icon :component="FlashOutline" />
              </template>
            </n-input>
          </n-gi>
          <n-gi><n-button attr-type="submit" @click="search" round> Go </n-button></n-gi>
        </n-grid>
      </n-gi>
    </n-grid>
    ...
    <n-grid :cols="1" style="height: 50%">
      <n-gi span="1">
        <svg ref="scatter" style="height: 100%"></svg>
      </n-gi>
    </n-grid>
  </n-card>
</template>
...
export default {
  ...
  methods: {
    search() {
      console.log(1, this.query, this.owner, this.repo);
      axios
